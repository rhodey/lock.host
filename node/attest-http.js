...
async function onHello(stream, cache, log) {
  ...
  let attestDoc = await attest(Buffer.from(server.publicKey), nonce)
...
module.exports = async function start(port, onError, log) {
  const target = 1 + port
  ...
  // http2 requires https in browsers
  ...
  server.on('stream', async (stream, headers) => {
    ...
    try {
      ...
      if (path.startsWith('/lockhost/hello')) {
        await onHello(stream, cache, log)
      } else if (path.startsWith('/lockhost/session')) {
        await onSession(stream, cache, target, log)
      } else if (path.startsWith('/lockhost/cert')) {
        writeHead(stream, 200)
